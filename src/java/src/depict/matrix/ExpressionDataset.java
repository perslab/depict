...
public class ExpressionDataset {
    ...
    public void adjustData(Integer sFixed) {
        
        // Linear regression
        SimpleRegression regression = new SimpleRegression();
        double[][] data = new double[nrProbes][2];
        for (int s=0; s<nrSamples; s++) {
            if (sFixed != s) {
                for (int probeID=0; probeID<nrProbes; probeID++) {
                    data[probeID][0] = rawData[probeID][sFixed];
                    data[probeID][1] = rawData[probeID][s];
                }
                ...
                // Predict all
                for (int probeID=0; probeID<nrProbes; probeID++) {
                    double residual = rawData[probeID][s] - regression.predict(rawData[probeID][s]);
                    rawData[probeID][s] = residual;

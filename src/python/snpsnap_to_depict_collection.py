...
# Yggdrasil, ld0.5 with chr x support, Phase 3: /tools/snpsnap/

depict_data_path = "/Users/tp/Drive/Projects/DEPICT/Code/DEPICT/data"
depict_gene_annotation_file = "%s/mapping_and_annotation_files/GPL570ProbeENSGInfo+HGNC_reformatted.txt"%depict_data_path
collection_file = "/Volumes/SANDISK256/depict-x/ld0.5_collection.EUR.incl_chrX.2015-07.tab.gz"
...
def get_nearest_gene_intervall_tree(depict_gene_annotation_file, depictgenes):
    ...
    with open (depict_gene_annotation_file,'r') as infile:
        for line in infile.readlines()[1:]:
            ...
            if words[ens_col] in depictgenes:
                tss = int(words[sta_col]) if words[str_col] == '1' else int(words[end_col])
                trees[words[chr_col]].insert_interval(Interval(tss, tss, value=words[ens_col])) if words[ens_col] in depictgenes and words[chr_col] in [str(x) for x in range(chr_start,chr_end+1,1)] else None
    return trees
...
def get_nearest(chrom, pos):
    ...
    if not gene_up:
        return gene_down[0].value
    
    # Assuming there will always be a gene_up if there is no gene_down (i.e. we are at the end of the chr)
    elif not gene_down:
        return gene_up[0].value

    # Test whether upstream or downstream gene
    return gene_up[0].value if abs(gene_up[0].start - pos) < abs(gene_down[0].start - pos) else gene_down[0].value
...
with open (depict_gene_annotation_file,'r') as infile:
    depictgenes = [line.strip().split("\t")[0] for line in infile.readlines()[1:]]

...
def run_depict(java_executable, depict_jar, background_data_path, locus_file, label, do_geneprio, do_gsea, do_tissue, ncores, analysis_path, reconstituted_genesets_file, depict_gene_annotation_file, tissue_expression_file, max_top_genes_for_gene_set, nr_repititions, nr_permutations, mhc_start_bp, mhc_end_bp, go_mapping_file, mgi_mapping_file, inweb_mapping_file, tissue_mapping_file, eqtl_mapping_file, eqtl_file, heap_size_in_mb, prioritize_genes_outside_input_loci, leave_out_chr, export_cofunc_and_exit, export_cofunc_type, export_cofunc_label):
	...
	def get_cmd(geneprio_flag, gsea_flag, tissue_flag):
		cmd = [java_executable,	"-Xms512M", "-Xmx{}M".format(heap_size_in_mb),"-XX:+UseParallelGC", '-XX:ParallelGCThreads=3', "-jar",
			...
			str(int(export_cofunc_and_exit)),# 24 Boolean exportCofuncAndExit
			export_cofunc_type,		# 25 String exportCofuncType 
			export_cofunc_label		# 26 String exportCofuncLabel
		]
		return cmd
